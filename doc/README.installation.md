...
```bash
conda env create -f badger/envs/badger-0.5.2.yml 
```

#### 2. Update the base environment with the proper dependencies of `badger-plots`
```bash
conda env update --name badger-0.5.2 --file badger/src/badger-plots/envs/badger-plots.yml 
```

#### 3. Install the badger command line program using pip
```bash
conda activate badger-0.5.2
pip install ./badger
```
...
echo BADGER_PLOTS_CONDA_ENV="badger-0.5.2" >> $R_ENVIRON
