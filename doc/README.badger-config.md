...
## `reference-genome`
- **Description**: Name of the reference genome used throughout the pipeline.
...
## `tempdir`
- **Description**: Set a custom temporary directory for methods and softwares requiring the creation of temporary file.
- **Allowed values**: (String) Any valid path to a directory.
## `archive`
Categorizes arguments related to the `badger archive` module.
### `archive-dir`
- **Description**: Set a custom directory where BADGER will archive its results, using the `badger archive` module.
- **Allowed values**: (String) Any valid path to a directory.
### `compress-level`
- **Description**: Specify a custom compression level for the [`xz`](https://linux.die.net/man/1/xz) compression algorithm. higher values will request for a higher compression rate impying a longer runtime, but a lighter disk-usage footprint.
- **Allowed values**: (Integer) An integer in the range $[0, 9]$
## `ped-sim`
Categorizes arguments related to pedigree simulations, and the `ped-sim` software
### `replicates`
- **Description**: Number of pedigree replicates for this given badger run. Increasing this value will optimize the overall runtime of BADGER. but will also linearly increase the number of required snakemake jobs. This behavior will be of little consequence on HPC and computer-cluster environments, but may become unmannageable on single-server environments. Values in the range $[1, 10]$, combined with the use of `loop-pipeline` are thus highly recommended if you plan to run BADGER on a desktop computer.
- **Allowed values**: (Integer) An integer in the range $[1, +\infty[$
### `filter-indels`
- **Description**: Apply indel filtration on the input `vcf` dataset that is given to `ped-sim` as a source of founder individuals during pedigree simulations. 
- **Allowed values**: (boolean) `True` or `False`
### `filter-maf`
- **Description**: Apply minor allele filtration on the input `vcf` dataset that is given to `ped-sim` as a source of founder individuals during pedigree simulations.  
- **Allowed values**: (boolean) `True` or `False`
### `filter-maf-threshold`
- **Description**: Minor allele frequency filtration threshold. This value is ignored if [filter-maf](#filter-maf) is set to `False`
- **Allowed values**: (float) a valid floating point value set between 0.0 and 1.0
### `data`
Categorizes keywords and arguments providing `ped-sim` with its required input datasets.
#### `codes`
- **Description**: Path to a user-created pedigree-codes file. Examples of these files can be found in the [`resources/ped-sim/ped-definition`](resources/ped-sim/ped-definition) directory. Detailled specifications on the format and purpose of these files can be found here: [pedigree-codes-files](README.ped-sim-config.md#pedigree-codes-files)
- **Allowed values**:  (String) Any valid path pointing to a pedigree-codes file.
#### `definition`
- **Description**: Path to a user-created pedigree definition file (`.def`). Examples of these files can be found in the [`resources/ped-sim/ped-definition`](resources/ped-sim/ped-definition) directory.
- **Allowed values**: (String) Any valid path pointing a *"`.def`"* pedigree definition file.
#### `map`
- **Description**: Path to a user-created ped-sim genetic map file. If set to `None`, BADGER will attempt to download and create a default file from the HapMapII recombination map. See the corresponding entry in `ped-sim`'s documentation here: [Map File](https://github.com/williamslab/ped-sim/tree/v1.4.2?tab=readme-ov-file#map-file-)
- **Allowed values**: (String) (Optional) Any valid path pointing to a valid *"`.map`"* genetic map file, or *`None`* (`~` character).
#### `interference`
- **Description**: Path to a user-created ped-sim *`.intf`* file. If set to `None`, BADGER will attempt to download a default file from `ped-sim`'s code repository. See the corresponding entry in `ped-sim`'s documentation here: [intf file](https://github.com/williamslab/ped-sim/tree/v1.4.2?tab=readme-ov-file#crossover-interference-model---intf-file)
- **Allowed values**: (String) (Optional) Any valid path pointing to a valid *`.intf`* interference file, or *`None`* (`~` character)
### `params`
Categorizes optional parameters and flags for the `ped-sim` program. These optional parameters are described in a dedicated section within ped-sim's documentation: [Other optional arguments](https://github.com/williamslab/ped-sim/tree/v1.4.2?tab=readme-ov-file#other-optional-arguments)
Note that this section does not currently map every optional arguments of `ped-sim` exhaustively.
#### `error_rate`
- **Description**: Sets `ped-sim`'s internl genotyping error rate (`--err_rate` parameter)
- **Allowed values**: (float) Any valid floating point value found in the range $[0, 1]$
#### `missingness`:
- **Description**: Sets `ped-sim`'s internal rate of missing genotype calls (`--missingness` parameter)
- **Allowed values**: (float) Any valid floating point value found in the range $[0, 1]$
#### `retain-extra`
- **Description**: Include randomly selected reference individuals as additional samples, which are not related to the main template pedigree file. (`--retain_extra`) This parameter may be useful to evaluate the impact of the number of available Unrelated individuals on the sensitivity of cohort-normalization kinship estimation methods, such as `READ` or `KIN`. 
- **Allowed values**: (Integer) a valid integer number, specifying the number of additional samples to include. Negative values (e.g.: `-1`) will have the impact of printing all of the available input samples.
#### `pop`
- **Description**: Select which 1000g-project' population should be used as a source for founder individuals within `ped-sim`'s pedigree simulations. A detailled list of the available population codes can be seen in IGSR's FAQ section here: [What do the population codes mean ?](https://www.internationalgenome.org/faq/what-do-the-population-codes-mean/)
- **Allowed values**: (String) Any valid 1000 genome population, or super-population label. 
#### `seed`
- **Description**: Provide `ped-sim` with a fixed RNG seed. If set to None (`~`), BADGER will pick a random seed for you, and store it within `results/meta/pipeline-metadata.yml`
- **Allowed values**: (integer) Any valid 64bit integer.

## `simulator`
- **Description**: Select which DNA fragment to use. `gargammel` is highly recommended for almost all intents and purposes, `dwgsim` can be useful for debugging purposes, or to simulate modern sequencing data.
- **Allowed values**: (String) Either *`"gargammel"`* or *`"dwgsim"`*
## `gargammel`
...
### `coverage`
- **Description**: Average simulated sequencing depth (X).
- **Allowed values**: (float) Any non-negative floating point value.
### `comp_endo`
- **Description**: Fractional composition of endogenous sequences.
- **Allowed values**: (float) Any valid floating point value in the range $[0, 1]$
### `comp_cont`
- **Description**: Fractional composition of modern human contaminating sequences. Note that this value should be equal to (1 - *[`comp_endo`](#com)*)
- **Allowed values**: (float) Any valid floating point value in the range $[0, 1]$
### `comp_bact`
- **Description**: Fractional composition of contaminating bacterial sequences. Note that setting this value to anything other than 0 will trigger BADGER to automatically download the example bacterial database proposed in gargammel's source code. 
- **Allowed values**: (float) Any valid floating point value in the range $[0, 1]$
### `pmd-model`
- **Description**: Select which post-mortem damage simulation model to use with `gargammel`.
...
- **Allowed values**: (String) *`"briggs"`* or *`"misincorporation"`*
### `misincorporation`
...
#### `file`
- **Description**: Path to a user-generated *`misincorporation.txt`* file. Note that these files can be generated by applying [`MapDamage`](https://ginolhac.github.io/mapDamage/) on a template sample.
- **Allowed values**: (String) Any valid path pointing to a valid *"`misincorporation.txt`"* file. Usable examples of such files can be found in the [`resources/gargammel/misincorporations/`](resources/gargammel/misincorporations/) directory of this repository.
#### `protocol`
- **Description**: Specify whether the misincorporation.txt file was gnerated from a double-strand or single-strand library.
- **Allowed values**: (String) *`"single"`* or *`"double"`*
### `briggs`
...
- **Allowed values**:
#### `nick-frequency`
- **Description**: Per-base nick-frequency rate ($\nu$ parameter.)
- **Allowed values**:  (float) Any valid floating point value in the range $[0, 1]$
#### `overhang-length`
- **Description**: length of single-stranded overhangs ($\lambda$ parameter)
- **Allowed values**: (float) Any valid floating point value in the range $[0, 1]$
#### `ds-deaminations`
- **Description**: Rate of double-stranded DNA deaminations ($\delta$ parameter)
- **Allowed values**: (float) Any valid floating point value in the range $[0, 1]$
#### `ss-deaminations`
- **Description**: Rate of single-stranded DNA deaminations ($\delta_{SS}$ parameter)
- **Allowed values**: (float) Any valid floating point value in the range $[0, 1]$
### `sizefreq`
- **Description**: Path to a user-generated fragment size frequency table. Usable exemples of such files may be found in the [`resources/gargammel/sizefreqs/`](resources/gargammel/sizefreqs/) directory of this repository. This file should be unheaded, and contain two columns
    - `<n>`: length of the fragment
    - `<prob>`: probability density of the given fragment length from occuring.
- **Allowed values**: (String) Any valid path pointing to an existing size-frequency distribution table. 
### `qshift`
- **Description**: Shift the error rate of reads by a factor of `Increase error rate for forward reads by a factor of ${1}/{(10^{{<factor>}\over{10}})}$. Notice that higher values will carry the effect of ***decreasing*** the error rate on both forward and reverse strands.
- **Allowed values**: (Integer) Any valid integer value in the range $[0, 93]$
### `params`
...
### `seed`
- **Description**: Provide `dwgsim` with a fixed RNG seed. If set to None (`~`), BADGER will pick a random seed for you, and store it within `results/meta/pipeline-metadata.yml`
- **Allowed values**: (integer) Any valid 64bit integer.
...
#### `min-overlap`
- **Description**: Only trim adapters having an overlap of $n$ nucleotides with its corresponding read (not counting ambiguous nucleotides). See the corresponding section in the documentation of `AdapterRemoval`: [`--minadapteroverlap`](https://adapterremoval.readthedocs.io/en/2.3.x/manpage.html#cmdoption-AdapterRemoval-minadapteroverlap)
- **Allowed values**: (integer) any positive non-zero integer representing a fragment length.
#### `min-length`
- **Description**: Discard any read shorter than the provided length following adapter trimming. See the corresponding section in the documentation of `AdapterRemoval`: [`--minlength`](https://adapterremoval.readthedocs.io/en/2.3.x/manpage.html#cmdoption-AdapterRemoval-minlength)
- **Allowed values**: (integer) any positive non-zero integer, representing a fragment length.
#### `min-quality`
- **Description**: Trim low-quality bases that are lower than the provided threshold. See the corresponding section in the documentation of `AdapterRemoval`: [`--minquality`](https://adapterremoval.readthedocs.io/en/2.3.x/manpage.html#cmdoption-AdapterRemoval-minquality)
- **Allowed values**: (integer) any positive non-zero integer, representing a PHRED scale base quality score [(Ewing and Green, 1998)](https://doi.org/10.1101/gr.8.3.186).
#### `qualitymax`
- **Description**: Specifies the maximum expected Phred score found within the input fastq files. Modifying this parameter should only be useful for debugging purposes and/or when modifying [gargammel:qshift](#qshift) See the corresponding section in the documentation of `AdapterRemoval`: [`--qualitymax`](https://adapterremoval.readthedocs.io/en/2.3.x/manpage.html#cmdoption-AdapterRemoval-qualitymax)
- **Allowed values**: (integer) any positive non-zero integer, representing a PHRED scale base quality score [(Ewing and Green, 1998)](https://doi.org/10.1101/gr.8.3.186).
#### `seed`
- **Description**: Provide `AdapterRemoval` with a fixed RNG seed. If set to None (`~`), BADGER will pick a random seed for you, and store it within `results/meta/pipeline-metadata.yml`. See the corresponding section in the documentation of `AdapterRemoval`: [`--seed`](https://adapterremoval.readthedocs.io/en/2.3.x/manpage.html#cmdoption-AdapterRemoval-seed)
- **Allowed values**: (integer) Any valid 64bit integer.
### `bwa`
...
#### `aligner`
- **Description**: Select the main alignment algorithm for `bwa`:
...
- **Allowed values**: (string) *`"aln"`* or *`"mem"`*
#### `collapsed-only`
- **Description**: Ignore and filter out uncollapsed reads before alignment when simulating paired-end data.
...
##### `seed-length`
- **Description**: Minimum seed length (this parameter is mapped to the `-l` argument of `bwa aln`). The provided integer value will require `bwa` to use subsequence of the corresponsing length as seed. However, If the provided value is ***larger*** than the query sequence, then seeding will be disabled altogether. This latter behavior is usually recommended in most use-cases when aligning aDNA whole-genome shotgun sequencing data.
- **Allowed values**: (integer) Any positive non-zero integer, representing a seed length (in bases).
##### `max-open-gap`
- **Description**: Maximum number of gap opens when aligning a fragment (i.e. a continuous indel sequence found along the fragment). This parameter is mapped to the `-o` argument of `bwa aln`. 
- **Allowed values**: (integer) Any positive non-zero integer, representing a maximum allowed number of occurences.
##### `max-seed-diff`
- **Description**: Maximum within-seed edit distance. This parameter is mapped to the `-k` argument of `bwa aln`.
- **Allowed values**: Any positive non-zero integer, representing a maximum number of allowed substitutions
##### `max-miss-prob`
- **Description**: This parameter is mapped to the `-n` argument of `bwa aln`, which can can either control the maximum wthin-fragment edit distance, if the provided value is an integer, or the maximum fraction of missing alignments if the provided value is a floating point number. In the latter case, the maximum allowed edit distance is chosen dynamically, according the length of the fragment.
- **Allowed values**: Either a floating point value in the range $[0, 1]$, representing a fraction of missing alignments, or an integer value in the range $[0, +\infty]$, representing the maximum allowed Levenstein distance. 
### `filter`
Categorises post-alignment quality filtration parameters.
#### `min-MQ`
- **Description**: Filter-out any fragment carrying a mapping quality that is lower than the provided threshold
- **Allowed values**: (integer) Any positive non-zero integer in the range $[0, 33]$, representing a PHRED33 mapping quality score
#### `min-length`
- **Description**: Filter-out any fragment with a length lower than the provided threshold
- **Allowed values**: (integer) Any positive non-zero integer in the range $[0, +\infty]$, representing a fragment length.
### `dedup`
Categorises arguments related to PCR and/or optical duplicates removal softwares.
#### `method`
- **Description**:  Select the main duplicate removal program:
...
- **Allowed values**: (string) one of *`"samtools"`*|*`"picard"`*|*`"dedup"`*
### `pmd-rescaling`
Categorises arguments related to post-mortem damage estimation and rescaling softwares
#### `rescaler`
- **Description**:  Select the main pmd-rescaling software to use. This pre-processing step is optional and may be replaced by a null value (*`~`*), to skip pmd-rescaling alltogether.
...
- **Allowed values**: (string)(optional) either *`"mapdamage"`*, *`"pmdtools"`*, or *`None`* (*`~`* character). 
#### `apply-masking`
- **Description**: Boolean specifying if putatively deaminated nucleotides should be masked using the software `pmd-mask`. Note that masking terminal deaminations may be applied on either non-rescaled or previously rescaled bam files, depending on whether or not `rescaler` was set to None`~`. In both cases, setting `apply-masking` to *`True`* will trigger the execution of `MapDamagev2` regardless, as `pmd-mask` currently requires the misincorporation frequencies estimates of this software as an input.
...
##### `rescale`
- **Description**: Specify whether or not pmd-rescaling should be applied. Setting this parameter to `False` may be useful to run mapdamage in order to only obtain PMD-estimates as a form of QC-validation.
- **Allowed values**: (boolean) either *`True`* or *`False`*.
##### `downsample`
- **Description**: Estimate deamination frequencies on a randomly selected fraction of the file (if the provided value is a floating point), or a fixed number of reads (if the provided value is an integer). This can speed up computation, but may return inaccurate estimates.
- **Allowed values**: (float) A floating point value in the range $[0, 1]$, representing a fraction of the number of reads within a given file, or a non-negative integer value in the range $[0, +\infty]$, representing a set number of reads.
##### `downsample-seed`
- **Description**: Provide `MapDamagev2` with a fixed RNG seed for donwsampling. If set to None (`~`), BADGER will pick a random seed for you, and store it within `results/meta/pipeline-metadata.yml`.
...
##### `threshold`
- **Description**: Filter-out reads carrying a PMD-score that is lower than the provided threshold. This parameter may be useful to filter-out reads originating from modern human contamination. Note that setting this parameter to a value that is less than or equal to *`-19999`* will disable PMD-score filtering alltogether.
- **Allowed values**: (integer) Any integer value in the range $[-\infty, +\infty]$, representing a LOD score for the presence of PMD contamination.
##### `mask-terminal-deams`
- **Description**: Mask a set number of putatively deaminated nucleotides along fragments.
- **Allowed values**: (integer) Any positive integer in the range $[0, +\infty]$, representing a number of nucleotides.
#### `pmd-mask`
...
##### `threshold`
- **Description**: Mask all `5'C` and `3'G` along the reads, until the estimated nucleotide misincorporation rate is lower than the provided threshold
- **Allowed values**: (float) Any floating point value in the range $[0, 1]$, representing a rate of misincorporation.
